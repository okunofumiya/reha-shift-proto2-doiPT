...
# ★★★ バージョン情報 ★★★
APP_VERSION = "proto.2.3.0_team_balance" # チーム均等化機能追加
...
def _create_summary(schedule_df, staff_info_dict, year, month, event_units, all_half_day_requests):
    ...
    for d in days:
        ...
        # 以下の役割名は固定ではなくなりましたが、サマリー表示のために残しています。
        day_info['回復期'] = sum(0.5 if sid in half_day_staff_ids else 1 for sid in work_staff_ids if staff_info_dict[sid].get('役割') == '回復期専従')
        day_info['地域包括'] = sum(0.5 if sid in half_day_staff_ids else 1 for sid in work_staff_ids if staff_info_dict[sid].get('役割') == '地域包括専従')
        day_info['外来'] = sum(0.5 if sid in half_day_staff_ids else 1 for sid in work_staff_ids if staff_info_dict[sid].get('役割') == '外来PT')
        if calendar.weekday(year, month, d) != 6: # ToDo: この休日判定も柔軟にすべき
...
def solve_shift_model(params):
    ...
    holiday_rules = config.get('holiday_rules', {})
    ...
    special_holidays.update(sundays) # 日曜日は常に特別休日
    if holiday_rules.get('saturday_is_special', False):
        saturdays = {d for d in days if calendar.weekday(year, month, d) == 5}
        special_holidays.update(saturdays)
    if holiday_rules.get('national_holiday_is_special', True):
        jp_holidays = {d.day for d in jpholiday.month_holidays(year, month)}
        special_holidays.update(jp_holidays)
    
    weekdays = [d for d in days if d not in special_holidays]
    ...
    # 役割に応じた休日設定
    sunday_off_roles = role_rules.get('sunday_off', [])
    special_holiday_off_staff = [s for s, info in staff_info.items() if info.get('役割') in sunday_off_roles]
    ...
    if params['h4_on']:
        for s in special_holiday_off_staff:
            for d in special_holidays: model.Add(shifts[(s, d)] == 0)

    if params['h5_on']:
        for s in staff: model.Add(sum(shifts[(s, d)] for d in special_holidays) <= 2)
    ...
    if any([params['s1a_on'], params['s1b_on'], params['s1c_on']]):
        for d in sundays: # 日曜日の人数目標は日曜日のみで維持
    ...
    # S3, S5, S6などの他のソフト制約もここに入る...（今回は省略せず記述）
    if params['s3_on']: # 外来同時休の制約
        # この制約は役割名が固定のため、config.jsonの役割設定に応じて動的にする必要がある
        gairai_staff = [s for s in staff if staff_info[s].get('役割') == '外来PT']
        for d in days:
    ...
    if params['s5_on']: # 回復期配置の制約
        kaifukuki_pt = [s for s in pt_staff if staff_info[s].get('役割') == '回復期専従']
        kaifukuki_ot = [s for s in ot_staff if staff_info[s].get('役割') == '回復期専従']
        for d in days:
    ...
    if params['s6_on']:
        ...
        total_weekday_units_by_job = {job: sum(int(staff_info[s]['1日の単位数']) * (len(weekdays) / num_days) * (num_days - 9 - sum(1 for r in requests_map.get(s, {}).values() if r in ['有','特','夏'])) for s in members) for job, members in job_types.items() if members}
    ...
    # --- チーム均等化のソフト制約 ---
    if team_rules.get('balance_enabled', False) and 'チーム' in staff_info[staff[0]]:
        all_teams = params['staff_df']['チーム'].dropna().unique()
        staff_by_team = {
            team: params['staff_df'][params['staff_df']['チーム'] == team]['職員番号'].tolist()
            for team in all_teams
        }
        penalty_weight = team_rules.get('balance_penalty', 5)
        for team, members in staff_by_team.items():
            if not members or len(members) < 2: continue
            daily_counts = [sum(shifts[(s, d)] for s in members) for d in days]
            min_count = model.NewIntVar(0, len(members), f'min_count_{team}')
            max_count = model.NewIntVar(0, len(members), f'max_count_{team}')
            model.AddMinEquality(min_count, daily_counts)
            model.AddMaxEquality(max_count, daily_counts)
            penalties.append(penalty_weight * (max_count - min_count))
...
with st.expander("▼ ルール検証モード（上級者向け）"):
    ...
    with h_cols[3]: params_ui['h4_on'] = st.toggle('H4: 特定役割の特別休', value=True, key='h4')
    with h_cols[4]: params_ui['h5_on'] = st.toggle('H5: 特別休の出勤上限', value=True, key='h5')
    st.markdown("---")
...
if create_button:
    if staff_file is not None and requests_file is not None:
        try:
            ...
            # config.jsonを読み込んでparamsに追加
            config_path = 'config.json'
            if os.path.exists(config_path):
                with open(config_path, 'r', encoding='utf-8') as f:
                    params['config'] = json.load(f)
            else:
                st.warning("config.jsonが見つかりませんでした。デフォルト設定で実行します。")
                params['config'] = {} # 空の辞書を渡す
            ...
            if missing_cols:
                st.error(f"エラー: 職員一覧CSVの必須列が不足しています。: {', '.join(missing_cols)}")
                st.stop()
            ...
            if is_feasible:
                ...
                def style_table(df):
                    special_holiday_days = {d for d in range(1, num_days + 1) if calendar.weekday(year, month, d) in [5, 6] or jpholiday.is_holiday(datetime(year, month, d))}
                    styler = df.style.set_properties(**{'text-align': 'center'})
                    for col in df.columns:
                        if col[0] in special_holiday_days:
